...
// Simple Profile Characteristic 1 Properties pM2.5 ON OFF
static uint8 simpleProfileChar1Props = GATT_PROP_READ | GATT_PROP_WRITE;

// Characteristic 1 Value
static uint8 simpleProfileChar1 = 0;

// Simple Profile Characteristic 1 User Description pM2.5 DATA
...
// Characteristic 3 Value   SET pM2.5 PERIOD
...
// Simple Profile Characteristic 5 Properties pM2.5 History notification on off
...
static uint8 simpleProfileChar10UserDesp[] = "OAD\0";
/*********************************************************************
...
bStatus_t SimpleProfile_GetParameter( uint8 param, void *value )
{
  ...
  switch ( param )
  {
      ...
      *((uint8*)value) = simpleProfileChar7;
      ...
      break;
      case SIMPLEPROFILE_CHAR9:
      *((uint8*)value) = simpleProfileChar9;
      break;
      case SIMPLEPROFILE_CHAR10:
        ...
        switch ( uuid )
        {
            ...
            if ( status == SUCCESS )
            {
                ...
                *pCurValue = pValue[0];
                ...
                else if( pAttr->pValue == &simpleProfileChar5 )
                {
                    notifyApp = SIMPLEPROFILE_CHAR5;           
                }
                else if( pAttr->pValue == &simpleProfileChar6 )
                {
                    notifyApp = SIMPLEPROFILE_CHAR6;           
                }
                else if( pAttr->pValue == &simpleProfileChar10 )
                {
            ...
            {
                uint8 *pCurValue = (uint8 *)pAttr->pValue;        
                osal_memcpy(pCurValue,pValue,SIMPLEPROFILE_CHAR8_LEN);
...
bStatus_t simpleProfile_NofifyChar4Data(uint8 *data,uint32 len)
{
    ...
    status=GATT_Notification( 0, &nData, FALSE );
      //impleProfile_SetParameter( SIMPLEPROFILE_CHAR4, SIMPLEPROFILE_CHAR4_LEN, data);
    if(SUCCESS!=status)
    {
        status = status;
    }
