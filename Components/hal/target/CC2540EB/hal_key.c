...
#define HAL_CHARGE_CPU_PORT_1_IF P1IF
...
void HalKeyInit( void )
{
  ...
  HAL_CHARGE_MONITOR_SEL &= ~(HAL_CHARGE_MONITOR_BIT); /* Set pin function to GPIO */
  HAL_CHARGE_MONITOR_DIR &= ~(HAL_CHARGE_MONITOR_BIT); /* Set pin direction to Input */
    
  P0INP |= PUSH1_BV;  /* Configure GPIO0 tri-state. */
  P2INP |= PUSH2_BV;  /* Configure GPIO 2tri-state. */
  P1INP |= PUSH2_BV;  /* Configure GPIO1 tri-state. */
#endif
  ...
  P1IFG &= ~BV(2);
#if 0
P1IF = 0; // ÖÐ¶Ï±êÖ¾Î»ÖÃ 0
P1SEL &= ~0x01; // °ÑP1.0µÄ¹¦ÄÜÉèÖÃÎª GPIO
P1DIR &= ~0x01; // °ÑP1.0ÉèÎªÊäÈë
P1IEN |= 0x01; // °ÑP1.0 µÄÖÐ¶Ï¹¦ÄÜ´ò¿ª
PICTL |= 0x02; // ÏÂ½µÑØ´¥·¢
IEN2 |= 0x10; // Ê¹ÄÜPort 1 µÄÖÐ¶Ï.
#endif
#if defined ( CC2540_MINIDK )
...
uint8 HalKeyRead ( void )
{
  ...
  if ((HAL_CHARGE_MONITOR_PORT & HAL_CHARGE_MONITOR_BIT))  /* Key is active low */
  {
    keys |= HAL_POWER_CHARGE;
  }
#endif
  return keys;
}
...
void halProcessKeyInterrupt (void)
{
  ...
  if (HAL_CHARGE_MONITOR_PXIFG & HAL_CHARGE_MONITOR_BIT)  /* Interrupt Flag has been set */
  {
      HAL_CHARGE_MONITOR_PXIFG = ~(HAL_CHARGE_MONITOR_BIT); /* Clear Interrupt Flag */
      valid = TRUE;
  }
#endif
  if (valid)
...
HAL_ISR_FUNCTION( halKeyPort1Isr, P1INT_VECTOR )
{
  ...
  if (HAL_CHARGE_MONITOR_PXIFG & HAL_CHARGE_MONITOR_BIT)
  {
